...
theme_thesis <- theme_minimal(base_size = 13, base_family = "serif") +
  theme(
...
treat_colors <- c("1" = "#0072B2", "0" = "#D55E00")


# Figure 1: Wage trends by quadrant
...
fig1 <- ggplot(wage_trends, aes(x = date, y = mean_wage,
                                 color = quadrant, group = quadrant)) +
  ...
  annotate("text", x = 2020.1, y = max(wage_trends$mean_wage) * 0.98,
           label = "COVID-19", hjust = 0, size = 3, color = "grey40",
           family = "serif") +
  scale_color_manual(values = quad_colors, labels = quad_labels) +
  scale_x_continuous(breaks = 2017:2025) +
  scale_y_continuous(labels = scales::dollar_format()) +
  labs(
    title = "Mean Real Hourly Wages by Task-Telework Quadrant",
    subtitle = "CPS-ORG, 2017-2025, real 2019 dollars (CPI-U-RS adjusted)",
    x = NULL, y = "Mean Real Hourly Wage ($2019)"
  ) +
  theme_thesis +
  guides(color = guide_legend(nrow = 2))

ggsave(file.path(FIG_DIR, "fig1_wage_trends_quadrant.pdf"),
       fig1, width = 10, height = 6.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig1_wage_trends_quadrant.png"),
       fig1, width = 10, height = 6.5, dpi = 300)


# Figure 1b: DID trends (treatment vs control)

did_trends <- did[, .(
  mean_log_wage = weighted.mean(log_wage, EARNWT), N = .N
), by = .(treated, year_quarter, YEAR, quarter)]
did_trends[, date := YEAR + (quarter - 1) / 4]
did_trends[, group := factor(treated, labels = c("Control (Non-Teleworkable)",
                                                   "Treatment (Teleworkable)"))]

fig1b <- ggplot(did_trends, aes(x = date, y = mean_log_wage,
  ...
  scale_x_continuous(breaks = 2017:2025) +
  labs(
    title = "Mean Log Wages: Treatment vs. Control (High-Abstract Occupations)",
    subtitle = "DID estimation sample, CPS-ORG, 2017-2025",
    x = NULL, y = "Mean Log Real Hourly Wage"
  ) +
  theme_thesis

ggsave(file.path(FIG_DIR, "fig1b_did_trends.pdf"),
       fig1b, width = 10, height = 6, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig1b_did_trends.png"),
       fig1b, width = 10, height = 6, dpi = 300)


# Figure 2: Event study (binary treatment)

es_data[, yq_label := {
  yr <- 2020 + event_time %/% 4
  qt <- (event_time %% 4) + 1
  qt_adj <- ifelse(event_time < 0, ((event_time %% 4) + 4) %% 4 + 1, qt)
  yr_adj <- 2020 + (event_time - (qt_adj - 1)) %/% 4
  paste0(yr_adj, "Q", qt_adj)
}]
...
fig2 <- ggplot(es_data, aes(x = event_time, y = estimate)) +
  ...
  labs(
    title = "Event Study: Effect of Telework Access on Abstract-Task Wages",
    subtitle = "Coefficients relative to 2019Q4 (reference period), 95% CIs",
    x = "Quarter Relative to 2020Q1",
    y = "Log Wage Differential (Treatment - Control)",
    caption = "Notes: Regression includes occupation, year-quarter, education, race, industry, and state FEs.\nSample restricted to high-abstract occupations. SEs clustered at occupation level."
  ) +
  theme_thesis +
  theme(axis.text.x = element_text(size = 7, lineheight = 0.9))

ggsave(file.path(FIG_DIR, "fig2_event_study.pdf"),
       fig2, width = 11, height = 6.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig2_event_study.png"),
       fig2, width = 11, height = 6.5, dpi = 300)


# Figure 3: Continuous treatment event study
...
fig3 <- ggplot(ct_es_data, aes(x = event_time, y = estimate)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Continuous Treatment Event Study",
    subtitle = "Telework feasibility (continuous) x event-time, relative to 2019Q4",
    x = "Quarter Relative to 2020Q1",
    y = "Marginal Effect of Telework Feasibility",
    caption = "Notes: Telework feasibility ranges from 0 to 1 (Dingel-Neiman continuous score).\nSample restricted to high-abstract occupations. SEs clustered at occupation level."
  ) +
  theme_thesis +
  theme(axis.text.x = element_text(size = 7, lineheight = 0.9))

ggsave(file.path(FIG_DIR, "fig3_continuous_event_study.pdf"),
       fig3, width = 11, height = 6.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig3_continuous_event_study.png"),
       fig3, width = 11, height = 6.5, dpi = 300)


# Figure 4: First-stage evidence
...
fig4 <- ggplot(first_stage, aes(x = reorder(quadrant_label, -pct_telework),
  ...
  labs(
    title = "First Stage: Actual Telework Adoption by Quadrant",
    subtitle = "CPS TELWRKPAY variable, October 2022 onwards",
    x = NULL, y = "Share Teleworking for Pay"
  ) +
  theme_thesis +
  theme(axis.text.x = element_text(size = 9))

ggsave(file.path(FIG_DIR, "fig4_first_stage.pdf"),
       fig4, width = 9, height = 6, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig4_first_stage.png"),
       fig4, width = 9, height = 6, dpi = 300)


# Figure 5: Heterogeneity coefficient plot
...
extract_coef <- function(model, label) {
  ...
  data.table(
    group = label, estimate = ct[idx[1], "Estimate"],
...
het_fig_list <- list(
  extract_coef(het_models$male, "Male"),
  extract_coef(het_models$female, "Female"),
  extract_coef(het_models$metro_only, "Metro"),
  extract_coef(het_models$nonmetro_only, "Non-Metro"),
  extract_coef(het_models$ba_plus, "BA+"),
  extract_coef(het_models$no_ba, "No BA"),
  extract_coef(het_models$union, "Union"),
  extract_coef(het_models$nonunion, "Non-Union")
)
...
fig5 <- ggplot(het_coefs, aes(x = estimate, y = group, color = dimension)) +
  ...
  scale_color_manual(values = c(
    "Gender" = "#0072B2", "Geography" = "#D55E00",
    "Education" = "#009E73", "Union Status" = "#CC79A7",
    "Age" = "#E69F00"
  )) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Heterogeneity in the Telework-Wage Effect",
    subtitle = "Coefficient on Treated x Post by subgroup, 95% CIs",
    x = "Estimated Effect on Log Wages", y = NULL,
    caption = "Notes: Each point is from a separate regression on the indicated subgroup.\nAll specifications include occupation, year-quarter, education, race, industry, and state FEs."
  ) +
  theme_thesis +
  theme(legend.position = "right",
        legend.title = element_text(face = "bold", size = 10))

ggsave(file.path(FIG_DIR, "fig5_heterogeneity.pdf"),
       fig5, width = 9, height = 5.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig5_heterogeneity.png"),
       fig5, width = 9, height = 5.5, dpi = 300)


# Figure 6: Robustness coefficient plot
...
extract_robust <- function(model, label, coef_name = "treated_x_post") {
  ...
  if (length(idx) == 0) return(NULL)
  data.table(
    spec = label, estimate = ct[idx[1], "Estimate"],
    se = ct[idx[1], "Std. Error"],
    ci_lo = ct[idx[1], "Estimate"] - 1.96 * ct[idx[1], "Std. Error"],
    ci_hi = ct[idx[1], "Estimate"] + 1.96 * ct[idx[1], "Std. Error"]
  )
...
fig6 <- ggplot(robust_coefs, aes(x = estimate, y = spec)) +
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Robustness of the Baseline DID Estimate",
    subtitle = "Coefficient on Treated x Post across specifications, 95% CIs",
    x = "Estimated Effect on Log Wages", y = NULL,
    caption = "Notes: Shaded band shows 95% CI of the preferred specification.\nPlacebo test uses pre-pandemic data (2017-2019) with fake treatment at 2018Q1."
  ) +
  theme_thesis

ggsave(file.path(FIG_DIR, "fig6_robustness.pdf"),
       fig6, width = 9, height = 6, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig6_robustness.png"),
       fig6, width = 9, height = 6, dpi = 300)


# Figure 7: Wage density

did[, period_label := fifelse(post == 0, "Pre-2020", "Post-2020")]
did[, treat_label := fifelse(treated == 1,
                              "Teleworkable (Treatment)",
                              "Non-Teleworkable (Control)")]
...
fig7 <- ggplot(did, aes(x = log_wage, fill = period_label, weight = EARNWT)) +
  ...
  labs(
    title = "Log Wage Distribution: Treatment vs. Control, Pre vs. Post",
    subtitle = "High-abstract occupations only, weighted by earnings weight",
    x = "Log Real Hourly Wage", y = "Density"
  ) +
  theme_thesis +
  theme(legend.position = "bottom")

ggsave(file.path(FIG_DIR, "fig7_wage_density.pdf"),
       fig7, width = 10, height = 5.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig7_wage_density.png"),
       fig7, width = 10, height = 5.5, dpi = 300)


# Figure 8: Progressive controls
...
spec_labels <- c(
  "m1" = "(1) OCC + YQ FE", "m2" = "(2) + Demographics",
  "m3" = "(3) + Educ + Race FE", "m4" = "(4) + Industry FE",
  "m5" = "(5) + State FE", "m6" = "(6) RPP-Adjusted",
  "m7" = "(7) Ind x YQ FE"
)
prog_coefs[, spec_label := spec_labels[spec]]
prog_coefs[, spec_label := factor(spec_label, levels = spec_labels)]
...
fig8 <- ggplot(prog_coefs, aes(x = spec_label, y = estimate)) +
  ...
  labs(
    title = "Stability of the DID Estimate Across Specifications",
    subtitle = "Coefficient on Treated x Post with progressive controls, 95% CIs",
    x = NULL, y = "Estimated Effect on Log Wages"
  ) +
  theme_thesis +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 9))

ggsave(file.path(FIG_DIR, "fig8_progressive_controls.pdf"),
       fig8, width = 9, height = 5.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig8_progressive_controls.png"),
       fig8, width = 9, height = 5.5, dpi = 300)


# Figure 9: Overlaid event studies (baseline vs saturated)
...
fig9 <- ggplot(es_combined, aes(x = event_time, y = estimate,
                                 color = spec, fill = spec)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Event Study: Baseline vs. Saturated Specification",
    subtitle = "Adding industry x year-quarter FE to absorb sector-specific pandemic shocks",
    x = "Quarter Relative to 2020Q1",
    y = "Log Wage Differential (Treatment - Control)",
    caption = "Notes: Both specifications include occupation, year-quarter, education, race, and state FEs.\nSaturated spec adds industry x year-quarter interactions. SEs clustered at occupation level."
  ) +
  theme_thesis +
  theme(axis.text.x = element_text(size = 7, lineheight = 0.9),
        legend.position = "bottom") +
...
ggsave(file.path(FIG_DIR, "fig9_event_study_overlay.pdf"),
       fig9, width = 11, height = 7, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig9_event_study_overlay.png"),
       fig9, width = 11, height = 7, dpi = 300)


# Figure 10: Honest DID sensitivity
...
fig10a <- ggplot(rm_dt, aes(x = Mbar, y = (lb + ub)/2)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Honest DID: Relative Magnitudes Sensitivity",
    subtitle = expression(paste("Robust CIs for first post-treatment period as ", bar(M), " varies")),
    x = expression(bar(M) ~ "(bound on post/pre violation ratio)"),
    y = "Confidence Interval for Treatment Effect",
    caption = "Notes: Rambachan & Roth (2023). At Mbar=0, standard parallel trends assumed.\nLarger Mbar allows proportionally larger post-treatment violations."
  ) +
  theme_thesis

ggsave(file.path(FIG_DIR, "fig10a_honest_did_rm.pdf"),
       fig10a, width = 8, height = 5.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig10a_honest_did_rm.png"),
       fig10a, width = 8, height = 5.5, dpi = 300)
...
fig10b <- ggplot(sd_dt, aes(x = M, y = (lb + ub)/2)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Honest DID: Smoothness Restriction Sensitivity",
    subtitle = expression(paste("Robust CIs under ", Delta^SD, " restriction as M varies")),
    x = "M (bound on change in slope of violation)",
    y = "Confidence Interval for Treatment Effect",
    caption = "Notes: Rambachan & Roth (2023). At M=0, linear pre-trend extrapolation assumed.\nLarger M allows more nonlinearity in the violation path."
  ) +
  theme_thesis

ggsave(file.path(FIG_DIR, "fig10b_honest_did_sd.pdf"),
       fig10b, width = 8, height = 5.5, device = cairo_pdf)
ggsave(file.path(FIG_DIR, "fig10b_honest_did_sd.png"),
       fig10b, width = 8, height = 5.5, dpi = 300)


# Figure A1: Metro vs. Non-metro Event Studies (Appendix)
...
if (file.exists(metro_es_path)) {
  ...
  figA1 <- ggplot(es_metro_combined, aes(x = event_time, y = estimate,
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    labs(
      title = "Event Study: Metro vs. Non-Metro Subsamples",
      subtitle = "Coefficients relative to 2019Q4, 95% CIs",
      x = "Quarter Relative to 2020Q1",
      y = "Log Wage Differential (Treatment - Control)",
      caption = "Notes: Separate event study regressions for metro and non-metro subsamples.\nAll specifications include occupation, year-quarter, education, race, industry, and state FEs."
    ) +
    theme_thesis +
    theme(axis.text.x = element_text(size = 7, lineheight = 0.9),
          legend.position = "bottom") +
  ...
  ggsave(file.path(FIG_DIR, "fig_A1_metro_nonmetro_es.pdf"),
         figA1, width = 11, height = 6.5, device = cairo_pdf)
  ggsave(file.path(FIG_DIR, "fig_A1_metro_nonmetro_es.png"),
         figA1, width = 11, height = 6.5, dpi = 300)
}
